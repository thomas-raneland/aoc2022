import java.util.ArrayList;
import java.util.Arrays;
import java.util.HashMap;
import java.util.List;
import java.util.Map;

public class Day22 {
    public static void main(String... args) {
        partI(testInput);
        partI(realInput);
        partII(testInput, testWarpRules());
        partII(realInput, realWarpRules());
    }

    private static void partI(String input) {
        String[] parts = input.split("\n\n");
        MonkeyMap map = parseMap(parts[0], Map.of());
        MonkeyPath path = parsePath(parts[1]);
        State state = map.startState();
        path.print();

        for (Step step : path.steps) {
            state = step.move(state, map);
        }

        map.print(state);
        System.out.println(state.password());
    }

    private static void partII(String input, Map<State, State> warpRules) {
        String[] parts = input.split("\n\n");
        MonkeyMap map = parseMap(parts[0], warpRules);
        MonkeyPath path = parsePath(parts[1]);
        State state = map.startState();

        for (Step step : path.steps) {
            state = step.move(state, map);
        }

        System.out.println(state.password());
    }

    private static MonkeyMap parseMap(String input, Map<State, State> warpRules) {
        Tile[][] tiles = new Tile[(int) input.lines().count()][input.lines().mapToInt(String::length).max().orElseThrow()];

        for (Tile[] row : tiles) {
            Arrays.fill(row, Tile.EMPTY);
        }

        int row = 0;

        for (String line : input.lines().toList()) {
            int col = 0;

            for (int c : line.chars().toArray()) {
                tiles[row][col++] = switch (c) {
                    case ' ' -> Tile.EMPTY;
                    case '.' -> Tile.OPEN;
                    case '#' -> Tile.WALL;
                    default -> throw new IllegalArgumentException();
                };
            }

            row++;
        }

        return new MonkeyMap(tiles, warpRules);
    }

    private static MonkeyPath parsePath(String input) {
        List<Step> steps = new ArrayList<>();
        int start = 0;

        for (int end = 0; end < input.length(); end++) {
            if (input.charAt(end) == 'L' || input.charAt(end) == 'R') {
                int nbr = Integer.parseInt(input.substring(start, end));
                steps.add(new Step(nbr, Rotation.NONE));
                steps.add(new Step(0, input.charAt(end) == 'L' ? Rotation.COUNTER_CLOCKWISE : Rotation.CLOCKWISE));
                start = end + 1;
            }
        }

        if (start < input.length()) {
            steps.add(new Step(Integer.parseInt(input.substring(start)), Rotation.NONE));
        }

        return new MonkeyPath(steps);
    }

    private enum Direction {
        RIGHT,
        DOWN,
        LEFT,
        UP;

        public Direction turnCounterClockwise() {
            return switch (this) {
                case RIGHT -> UP;
                case UP -> LEFT;
                case LEFT -> DOWN;
                case DOWN -> RIGHT;
            };
        }

        public Direction turnClockwise() {
            return switch (this) {
                case RIGHT -> DOWN;
                case DOWN -> LEFT;
                case LEFT -> UP;
                case UP -> RIGHT;
            };
        }

        public Direction reverse() {
            return switch (this) {
                case RIGHT -> LEFT;
                case LEFT -> RIGHT;
                case UP -> DOWN;
                case DOWN -> UP;
            };
        }
    }

    private static record State(int x, int y, Direction facing) {
        long password() {
            return 1000L * y + 4L * x + switch (facing) {
                case RIGHT -> 0;
                case DOWN -> 1;
                case LEFT -> 2;
                case UP -> 3;
            };
        }
    }

    private static record MonkeyMap(Tile[][] tiles, Map<State, State> warpRules) {
        void print(State state) {
            int y = 1;

            for (Tile[] row : tiles) {
                int x = 1;

                for (Tile col : row) {
                    if (state.x == x && state.y == y) {
                        System.out.print(switch (state.facing) {
                            case UP -> '^';
                            case DOWN -> 'v';
                            case LEFT -> '<';
                            case RIGHT -> '>';
                        });
                    } else {
                        System.out.print(col.character());
                    }

                    x++;
                }
                System.out.println();
                y++;
            }
        }

        State startState() {
            Tile[] firstRow = tiles[0];

            for (int i = 0; i < firstRow.length; i++) {
                if (firstRow[i] == Tile.OPEN) {
                    return new State(i + 1, 1, Direction.RIGHT);
                }
            }

            throw new IllegalStateException();
        }

        private boolean isOpen(int x, int y) {
            return tiles[y - 1][x - 1] == Tile.OPEN;
        }

        State walk(State state) {
            if (warpRules.containsKey(state)) {
                State newState = warpRules.get(state);
                return isOpen(newState.x, newState.y) ? newState : state;
            }

            int row = state.y - 1;
            int col = state.x - 1;

            switch (state.facing) {
            case RIGHT -> {
                col++;

                if (col > tiles[0].length - 1 || tiles[row][col] == Tile.EMPTY) {
                    col = 0;

                    while (tiles[row][col] == Tile.EMPTY) {
                        col++;
                    }
                }
            }
            case DOWN -> {
                row++;

                if (row > tiles.length - 1 || tiles[row][col] == Tile.EMPTY) {
                    row = 0;

                    while (tiles[row][col] == Tile.EMPTY) {
                        row++;
                    }
                }
            }
            case LEFT -> {
                col--;

                if (col < 0 || tiles[row][col] == Tile.EMPTY) {
                    col = tiles[0].length - 1;

                    while (tiles[row][col] == Tile.EMPTY) {
                        col--;
                    }
                }
            }
            case UP -> {
                row--;

                if (row < 0 || tiles[row][col] == Tile.EMPTY) {
                    row = tiles.length - 1;

                    while (tiles[row][col] == Tile.EMPTY) {
                        row--;
                    }
                }
            }
            }

            int x = col + 1;
            int y = row + 1;
            return isOpen(x, y) ? new State(x, y, state.facing) : state;
        }
    }

    private enum Tile {
        EMPTY,
        OPEN,
        WALL;

        char character() {
            return switch (this) {
                case EMPTY -> ' ';
                case OPEN -> '.';
                case WALL -> '#';
            };
        }
    }

    private static record MonkeyPath(List<Step> steps) {
        void print() {
            for (Step s : steps) {
                if (s.distance > 0) {
                    System.out.print(s.distance);
                }

                if (s.rotation != Rotation.NONE) {
                    System.out.print(s.rotation == Rotation.COUNTER_CLOCKWISE ? "L" : "R");
                }
            }

            System.out.println();
        }
    }

    private static record Step(int distance, Rotation rotation) {
        public State move(State state, MonkeyMap map) {
            for (int i = 0; i < distance; i++) {
                state = map.walk(state);
            }

            return rotation.rotate(state);
        }
    }

    private enum Rotation {
        NONE,
        COUNTER_CLOCKWISE,
        CLOCKWISE;

        public State rotate(State state) {
            return switch (this) {
                case NONE -> state;
                case COUNTER_CLOCKWISE -> new State(state.x, state.y, state.facing.turnCounterClockwise());
                case CLOCKWISE -> new State(state.x, state.y, state.facing.turnClockwise());
            };
        }
    }

    private static Map<State, State> testWarpRules() {
        int sideLength = 4;
        Map<State, State> rules = new HashMap<>();
        addYYRule(12, 1, Direction.RIGHT, 12, 9, Direction.LEFT, rules, sideLength, 1);
        addYXRule(12, 5, Direction.RIGHT, 16, 9, Direction.DOWN, rules, sideLength, -1);
        addYXRule(1, 5, Direction.LEFT, 13, 12, Direction.UP, rules, sideLength, 1);
        addXXRule(1, 8, Direction.DOWN, 12, 12, Direction.UP, rules, sideLength, -1);
        addXXRule(1, 5, Direction.UP, 12, 1, Direction.DOWN, rules, sideLength, -1);
        addYXRule(9, 1, Direction.LEFT, 5, 5, Direction.DOWN, rules, sideLength, 1);
        return rules;
    }

    private static Map<State, State> realWarpRules() {
        int sideLength = 50;
        Map<State, State> rules = new HashMap<>();
        addYXRule(100, 51, Direction.RIGHT, 101, 50, Direction.UP, rules, sideLength, 1);
        addYYRule(100, 101, Direction.RIGHT, 150, 50, Direction.LEFT, rules, sideLength, -1);
        addYXRule(50, 151, Direction.RIGHT, 51, 150, Direction.UP, rules, sideLength, 1);
        addXXRule(1, 200, Direction.DOWN, 101, 1, Direction.DOWN, rules, sideLength, 1);
        addYXRule(1, 151, Direction.LEFT, 51, 1, Direction.DOWN, rules, sideLength, 1);
        addYXRule(51, 51, Direction.LEFT, 1, 101, Direction.DOWN, rules, sideLength, 1);
        addYYRule(1, 101, Direction.LEFT, 51, 50, Direction.RIGHT, rules, sideLength, -1);
        return rules;
    }

    @SuppressWarnings("SameParameterValue")
    private static void addXXRule(int fromStartX, int fromY, Direction fromDirection, int toStartX, int toY,
                                  Direction toDirection, Map<State, State> rules, int length, int positive) {
        for (int i = 0; i < length; i++) {
            rules.put(new State(fromStartX + i, fromY, fromDirection), new State(toStartX + i * positive, toY, toDirection));
            rules.put(new State(toStartX + i * positive, toY, toDirection.reverse()),
                    new State(fromStartX + i, fromY, fromDirection.reverse()));
        }
    }

    private static void addYYRule(int fromX, int fromStartY, Direction fromDirection, int toX, int toStartY,
                                  Direction toDirection, Map<State, State> rules, int length, int positive) {
        for (int i = 0; i < length; i++) {
            rules.put(new State(fromX, fromStartY + i, fromDirection), new State(toX, toStartY + i * positive, toDirection));
            rules.put(new State(toX, toStartY + i * positive, toDirection.reverse()),
                    new State(fromX, fromStartY + i, fromDirection.reverse()));
        }
    }

    private static void addYXRule(int fromX, int fromStartY, Direction fromDirection, int toStartX, int toY,
                                  Direction toDirection, Map<State, State> rules, int length, int positive) {
        for (int i = 0; i < length; i++) {
            rules.put(new State(fromX, fromStartY + i, fromDirection), new State(toStartX + i * positive, toY, toDirection));
            rules.put(new State(toStartX + i * positive, toY, toDirection.reverse()),
                    new State(fromX, fromStartY + i, fromDirection.reverse()));
        }
    }

    private static final String testInput = """
                    ...#
                    .#..
                    #...
                    ....
            ...#.......#
            ........#...
            ..#....#....
            ..........#.
                    ...#....
                    .....#..
                    .#......
                    ......#.
                        
            10R5L5R10L4R5L5""";

    private static final String realInput = """
                                                              ..........#.#...........#.#...#..#......#.#.............#.....#...#...............#.....#......#..#.
                                                              ##..#..........#........#....#..............#...........#.....#............#...........#..#.........
                                                              .....#..........................#.........#..##...#..#.....#..................#.....................
                                                              .....#..................................................#.#..........#...................#.........#
                                                              ....#.....................#..#.........#..#.................##......................................
                                                              ...........................#..........#..#....#......................#.......#..#......#...........#
                                                              .......#..........#...................#...............#....##........#....#.#.........#....#..#.....
                                                              ......#.....#.......#.............#.........................#............................#.#....#...
                                                              ..#......#...#........................#............#.......#...................#..#.....#...........
                                                              .##........##..#......#..........#.......#...#..#...........................#....#...#.........#....
                                                              .#................##........#..............#.#..#.#........#.........#.........#......##............
                                                              ......................##......#....#........#.##....................#.#....##....##...............#.
                                                              #..................###...#........#..................#......#..#..#...#...#...#.....................
                                                              ....#.....#....#..#..#......#.......#...............#......#........................................
                                                              ........#.....#.......#......##..........#..........#........#....#..#....................#.......##
                                                              ........#.............#..........#..##.....#..............##...............................#.#.#....
                                                              .........................#.............#......#.....#.........#..........#.................#........
                                                              #.............#.........................................##......#...#..........#....................
                                                              ........#.#......#........................#....#.....#.........#.#......#.....##....................
                                                              ................................#.................#......##..#...#..#..#........##...##......#..#...
                                                              .......#........#..#....#.....#.#..........................#........................................
                                                              .........#.........#...............................#.......#.....................#...............#..
                                                              .................#....#..........................##............................#........#...........
                                                              .#...#........................#...#..#.....................#....#.........................##.#......
                                                              .......##....#.#....................#...#.....#..#..#.#.........#.........#.............#....#......
                                                              ......#..#......................#.#...#.#.......................#...#..#...................#...#....
                                                              ............#....##...........#..........#.......#..#..#....#......................................#
                                                              ..#....................#...#.........#.#.....#........#.#...#........................##.............
                                                              ..............#..#......##.......#....#.......#.....#........#........##..##.............#..........
                                                              ......##..#..........#........#...................#....................................#....#....#.#
                                                              .........................#.....##...##....#........#...#........#........#..................#.#.....
                                                              ......#.....#..................#.....#.......#.#...........#....##................#..#.....#.#......
                                                              ......#.#..#.................#..............................................#...............#....#..
                                                              ...#.....#.....#............#..............................#...#..#..............##.................
                                                              ......#........##.#..#..........#....##.#......#.........#................................#....#....
                                                              ...#.#...........................#....#.....#..........................#.............#.....#.....#..
                                                              .....................#.#.#.........................#............#....#.........#.........#.........#
                                                              ....#....#...................................#.........................#......#.........#...........
                                                              ................#...#............#.#...#..#.............#.#......................#...#......##....#.
                                                              ...........................#....#.............#..................#....###..........#...............#
                                                              ...#.................#..#.....#.............#.......#................#..#......................#.#..
                                                              ................#.......#.........#....#.......#......#..............#..#.#.......#...#......#......
                                                              ....##.#........#...#.........#.......................#...........##........##..#...................
                                                              .#......#...............#.....#.............#...........................#..#..#....#.........#......
                                                              ................#.....##.......#...........#..#........#.#....#......#..........................#..#
                                                              ...#......#..#......#....#........................#............#...#..............##................
                                                              .#...........#................#..#.....................#...............#........#...................
                                                              .....#...........#..............#...#......#.##........#...........#....#..................#........
                                                              #..#.............................#......#.....#........#.............#..#.#.#...#.#.................
                                                              #.................#....#..........#.....#.............................#.#.........#.................
                                                              .......#.....#..#........#..#....#...#..#.........
                                                              ........................#.............##.........#
                                                              ...#.....#........#......#.......#.#...#..........
                                                              .......#..........#...............................
                                                              ....#..#......................................#...
                                                              .....#..........#..##...#.............#..........#
                                                              #....#......#..#.#....#.......#.......#...........
                                                              ..................................................
                                                              ...#.....#..............#.............#....#.....#
                                                              ..............#.................#............#...#
                                                              ..................................................
                                                              ................#..............##..##........#..#.
                                                              ....#.............................#...............
                                                              .....##......#...##.................#.......#.....
                                                              .....#...#.................#...#......#...........
                                                              ........#.............#..............#.#..........
                                                              ........#.........#.....................#.........
                                                              .....#....#.......#..##....#............#.........
                                                              .....................#....###...#..#.#.....#......
                                                              ...#...........#................#.................
                                                              .##...#..#...#....#..#..#.........................
                                                              .....#.........#.#.....#...#..##.......##....#....
                                                              .....#..#.................#.........#.......#.....
                                                              ................#...#....................#........
                                                              ...#....#.............#..#......#.................
                                                              .........#..###.............................#..#..
                                                              .........#...#.........#..........................
                                                              ........#.......#..........#...#...#......#.......
                                                              ...##..#.#.....##...............#.#............#..
                                                              ..#.....................##......#.............#...
                                                              ....#...................#.........................
                                                              ..............#...............................#...
                                                              .....#.#.....#..........................#.##.#....
                                                              .....................#..........#....#....#......#
                                                              .....................#.....................##.....
                                                              ........#........#.#............#.##..#..#.......#
                                                              .....#...............#...#..............#.........
                                                              ................#....#.....#......................
                                                              .............##..............#....................
                                                              ..#.......#................#.......#...........#..
                                                              ...#............#.............#...................
                                                              #........#...............#....##..............#...
                                                              .................................#.#.#......#.....
                                                              .........................#.........##......#....#.
                                                              ...#..#........#.#...........#..........#...##.#..
                                                              ...........##..##......#.................#......#.
                                                              ......##.#...........................#.......#...#
                                                              ............#..#..................................
                                                              .............#..................#.......#...#.....
                                                              .......................#......................#...
            .............#.#...#......##..#..##...#.....#...#....##......#.................#...........##...#...
            ....#....#....................#..........##..#........#...#.....#.................................#.
            ...............................#..........#.....#.....................#......#...##........#....#...
            .....#............................#......#.......................#..............................##..
            .#....#.#.........................#........#........##...#..........................###.............
            .#...#.........#..................#..........#.............#....................#...................
            ..................#.#............#...#..##.....................#.........#........#....#............
            #................#.............................#.#.................#................................
            .........................#..................#.#..###................................................
            .....................................#........##..#.#....##.................................##......
            .........#...........#......................##.#.................#.....#....#........###............
            ........#.....#...........##......#.....#....#.##.##.#..##..................#......#................
            .......##....#...................#...#...........#......#........#........#...........#.....#.......
            .#.....#......#............#......................#......................#......................#...
            .........................#.........##...........................#..#.....#..#...#.#..........#......
            ...............#..............................#...........#.......................#...............#.
            .....#............................#....#........#...................#....#..#.................#.....
            ..#....................#.........#......#............................#..#..............#........#...
            ..#.#...#................................................##......#......#.#..............#..........
            ..#..#...#....#........#.........#.....#.........#....#...#.......#....#..........#...#.............
            .................##......##.....#.....##..#....#...#.................................#.#...#...#....
            .........#.#....................................#..#.......##.....................#......#.#..#.....
            #.#..........#.........##..#............#..............#.....#...#.........................#........
            .....#.....................#......#...................##...........#.............#........#.#....#.#
            ....#.........#.#.....#..#..................#................................#.........#....#......#
            ...........#....#.....#......#.#.........................#....#.....................................
            ...#....#.#.....#........#....#..........#..............#...#...#........#.............#..#.........
            ..............#......#.....##.............#......#............................#............#..#.....
            .#....................#.........#........#................#......##....#...................#........
            ...........#........#........#.....................................##......#.......#................
            ...........#...#...#..........##............##......#.....#....#.....................#........#.....
            .....................#.................#....................#...............#.................#.....
            ....................................##...........#.#..#.##.......#..#.........#.....................
            ............#..............................................#..##................#.#..#............#.
            ...##....#.#...#.....#..........#....#........#.###....#...##...##..#....##....#...#.........##....#
            .......................#....#....#...#....##...............................#....#...###.............
            ........#....#..##.............................##.....#....##..#...........#.#......#...............
            ...........#....#...................#........##...##...............#.#..................#.........#.
            .....................#..#..........................#.............#......#.............#...........#.
            ...........#.................#..........##....................................#.......#....#..#..#..
            ..................#.....#.#....#.......#.#.........#...................#...##................#.#....
            ........#..................#.................#.....#.....#...........#.......#........#.#...........
            #.......#......#.........#....#..................#..#...........#........#..#....#..................
            .............#.......#..................#......#...#............#.#.#.#...........#.....##.#........
            #....#.#................#....#...........#.#......................#................................#
            ....#.#...........................#.###...#...#.....#...................#....#....##...#............
            ....#.#.........#........#.........#........#.#.......##.......##.....#........#....................
            ......#.....#...#...........#..#..........#......................#....................#.............
            ....#..........................#...........##....#..#...............#....#......#.................#.
            ..##.................#........#.....#..................#...#...#...................#................
            .....#....##............#.......#.....#...........
            ....#.#.......#......#.#...........##.......#.#...
            .................................................#
            #.........#.....#...#..........#...............#..
            ......#.............#.......#.......#............#
            #........#....#....#....#............#....#....#..
            .......#.#......#.#.........#.........#.......#.#.
            ...#......................#....................#.#
            .............................#....................
            #..........#..#............#..#.#............#....
            ..#..............................#....#...........
            #...............................#...#......#......
            ...............#........#.........................
            ..................#......#..............#.........
            ...........#................#.......#..#...#..#...
            .........#......#...........#....#..............#.
            ...#........#..#.........#...#.......#...#........
            .....................#.........................#..
            #..................#.........#..#..#..#...........
            ..........#.#.#.........#..............#.....##...
            ...........#........#............................#
            ...................#.#...#............#...........
            .........#.#.....#..#.#....#.........#............
            ................#...#.............................
            .............#..#.#.......#.#..................#.#
            ..#.................#...........#...........#.....
            ..........#.#...........................#........#
            ......................#.................#.........
            ....#.#.................................#.........
            ..................#.#..........#..................
            .................##.....#.........................
            ......................................#...........
            ......#...............#.........#.............#...
            ..............#..........................#......#.
            ...........#.............###...............##.....
            .....................##.#..........#..#........#..
            ....................##.......#....................
            .......#....#........#.#....#....#...#.#....##....
            ..#............#..............#..#.....##......#..
            .........................#..............#.........
            ...................##.........#...#...............
            ....##............#......#..........#....#........
            ..#.....#..........#...........#...........#...#..
            ....#.........#........#.#.#.........#........#..#
            ............#...........................#.......#.
            ..##.#...#..................................#..#..
            .........#.........#........#..........#..........
            .##.......#...........#.#..#...#...#..............
            ..#..............#.##.#.#.....#...#.#...........#.
            .......#......#...............#..#..............##
                        
            """;
}
